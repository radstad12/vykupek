...
function App(){
  ...
  useEffect(()=>{
    const unsubscribe = onAuthStateChanged(auth, user => setSession(user))
    return unsubscribe
  },[])
  ...
  useEffect(()=>{
    ...
    const productsQuery = admin
      ? query(productsRef, orderBy('sort_order', 'asc'))
      : query(productsRef, where('active', '==', true), orderBy('sort_order', 'asc'))
  ...
  const selected = useMemo(
    ()=>products.map(p=>({product_id:p.id,quantity:Number(qty[p.id]||0),unit_price:Number(p.unit_price||0)}))
      .filter(x=>x.quantity>0),
    [products,qty]
  )
  const localTotal = selected.reduce((sum,x)=>sum + x.quantity * x.unit_price, 0)

  async function calculate(){
    if(!selected.length){setQuote(null);return}
    setQuote({subtotal:localTotal,bonus:0,total:localTotal})
  }
...
function Public({products,qty,setQty,calculate,quote,loading,message,selected}){
 ...
 {quote&&<div className="quote"><div><span>ORIENTAČNÍ VÝKUP</span><strong>{Number(quote.total).toLocaleString('cs-CZ')} $</strong></div>{Number(quote.bonus)>0&&<div className="bonus"><Sparkles/> Bonus +{Number(quote.bonus).toLocaleString('cs-CZ')} $</div>}<small>Finální cena je potvrzena při osobním převzetí zboží. {''}</small></div>}
 </section></main>
}
...
function Admin({products,reload,session,login,message,setMessage}){
 ...
 async function addProduct(e){
   ...
   try {
     ...
     setForm({name:'',category:'',unit_price:'',sort_order:0,active:true})
     setMessage('Produkt uložen a synchronizován.')
   } catch(error) {
     setMessage(error.message)
   } finally {
     setSaving(false)
 ...
 async function addBonus(e){
   ...
   try {
     await addDoc(collection(db,'bonuses'), {
       threshold:Number(bonus.threshold),
       bonus_type:bonus.bonus_type,
       bonus_value:Number(bonus.bonus_value),
       active:true,
       createdAt:serverTimestamp()
     })
     setMessage('Bonus uložen.')
   } catch(error) {
     setMessage(error.message)
   }
 }
